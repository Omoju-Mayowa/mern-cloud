...
const PostItem = ({postID, category, title, description, authorID, thumbnail, videoUrl, createdAt, likesCount = 0, likedBy = [], ...props}) => {
  ...
  const assetsBase = import.meta.env.VITE_API_ASSETS_URL || 'https://5df0a2941a74cae5cfbfcdd3744c8161.r2.cloudflarestorage.com/mern'
  const safeThumbnail = thumbnail || 'default-avatar.png'
  const imageUrl = safeThumbnail && (safeThumbnail.startsWith('http://') || safeThumbnail.startsWith('https://')) ? safeThumbnail : `${assetsBase}/uploads/${safeThumbnail}`
